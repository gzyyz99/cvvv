    ...
    <div className="space-y-6">
          ...
          {[
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm transition-all duration-200 ${
                filter === tab.value
                  ? "bg-gradient-to-r from-cyan-500/25 to-blue-500/20 border border-cyan-500/35 text-white font-medium"
            ...
            >
              ...
              <span
                className={`px-1.5 py-0.5 rounded-full text-xs ${
                  filter === tab.value
                    ? "bg-cyan-500/25 text-indigo-300"
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-10 py-2.5 text-gray-200 placeholder-gray-600 text-sm outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-indigo-500/20 transition-all"
        />
        {search && (
          <button
            onClick={() => setSearch("")}
            ...
            <div key={comment.id} className="relative group">
              ...
              >
                <div className="flex items-start gap-3 sm:gap-4">
                  {/* Avatar */}
                  <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-cyan-500/20 to-blue-500/20 border border-white/10 flex items-center justify-center shrink-0">
                    <img
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {comment.is_pinned && (
                        <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-cyan-500/15 border border-cyan-500/25 text-indigo-300 text-xs">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button
                      ...
                      className={`p-2 rounded-lg border transition-all duration-200 ${
                        comment.is_pinned
                          ? "border-cyan-500/30 bg-cyan-500/10 text-indigo-400 hover:bg-cyan-500/20"
                          : "border-white/10 text-gray-500 hover:text-indigo-400 hover:border-cyan-500/25"
                      }`}
                    >
                      {comment.is_pinned ? (
                ...
                p === "..." ? (
                  ...
                  <button
                    ...
                    className={`min-w-[32px] h-8 px-2 rounded-lg text-xs border transition-all duration-200 ${
                      page === p
                        ? "bg-cyan-500/20 border-cyan-500/40 text-indigo-300 font-medium"
...
function highlightMatch(text, query) {
  ...
  return parts.map((part, i) =>
    regex.test(part) ? (
      <mark key={i} className="bg-cyan-500/30 text-indigo-200 rounded px-0.5">
        {part}
      </mark>
    ) : (
      part
